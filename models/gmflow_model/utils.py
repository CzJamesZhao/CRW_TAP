...
from models.gmflow_model.position import PositionEmbeddingSine
...
def feature_add_position(feature0, feature1, attn_splits, feature_channels):
    ...
    if attn_splits > 1:  # add position in splited window
        ...
        position0 = pos_enc(feature0_splits)
        position1 = pos_enc(feature1_splits)

        if torch.is_autocast_enabled():
            position0 = position0.to(feature0_splits.dtype)
            position1 = position1.to(feature1_splits.dtype)
...
def normalize_imgs(images, divideby255=True):
    # images: (B, T, C, H, W)
    ...
    if divideby255:
        ...
        div = 1.0
    images = (images / div - mean) / std

    return images

def unnormalize_imgs(images):
    # images: (B, C, H, W)
    # loaded images are in normal distribution of ImageNet
    # unnormalize by ImageNet mean and std to range [0, 1]
